...
from src.sru_queryer._base._sru_explain_dict_parser import SRUExplainDictParser
...
# Format returned by SRUExplainDictParser's '_parse_config_info'
...
class TestSRUExplainDictParser(unittest.TestCase):
    ...
    def test_parse_context_set_and_index_info_from_xml_alma(self):
        with open (TestFiles.explain_response_alma, "rb") as f:
            ...
            sru_dict_parser = SRUExplainDictParser(xml_unprocessed_dict, driver=alma_driver)
            xml_dict = sru_dict_parser._parse_raw_index_info_from_dict()
    ...
    def test_parse_context_set_and_index_info_from_xml_loc(self):
        with open (TestFiles.explain_response_loc, "rb") as f:
            ...
            sru_dict_parser = SRUExplainDictParser(xml_unprocessed_dict, driver=loc_driver)
            xml_dict = sru_dict_parser._parse_raw_index_info_from_dict()
    ...
    def test_parse_context_set_and_index_info_from_xml_gapines(self):
        with open (TestFiles.explain_response_gapines, "rb") as f:
            ...
            sru_dict_parser = SRUExplainDictParser(xml_unprocessed_dict, driver=gapines_driver)
            xml_dict = sru_dict_parser._parse_raw_index_info_from_dict()
    ...
    def test_parse_sru_version_from_dict_alma(self):
        with open (TestFiles.explain_response_alma, "rb") as f:
            ...
            sru_dict_parser = SRUExplainDictParser(xml_unprocessed_dict, driver=alma_driver)
            version = sru_dict_parser._parse_sru_version_from_dict()
    ...
    def test_parse_schema_info_from_xml_gapines(self):
        with open (TestFiles.explain_response_gapines, "rb") as f:
            ...
            sru_dict_parser = SRUExplainDictParser(xml_unprocessed_dict, driver=gapines_driver)
            xml_dict = sru_dict_parser._parse_raw_schema_info_from_dict()
            xml_dict = sru_dict_parser._parse_schema_info(xml_dict)
    ...
    def test_parse_config_info_from_xml_alma(self):
        with open (TestFiles.explain_response_alma, "rb") as f:
            ...
            sru_dict_parser = SRUExplainDictParser(xml_unprocessed_dict, driver=alma_driver)
            xml_dict = sru_dict_parser._parse_raw_config_info_from_dict()
    ...
    def test_parse_available_context_set_and_index_info_from_xml_dict_alma(self):
        with open (TestFiles.alma_raw_indexes, "r") as f:
            ...
            sru_dict_parser = SRUExplainDictParser({}, driver=alma_driver)
            available_context_set_and_index_info = sru_dict_parser._parse_available_context_sets_and_index_info(explain_index_json)
    ...
    def test_parse_available_context_set_and_index_info_from_xml_dict_gapines(self):
        with open(TestFiles.gapines_raw_indexes, "r") as f:
            ...
            sru_dict_parser = SRUExplainDictParser({}, driver=gapines_driver)
            available_context_set_and_index_info = sru_dict_parser._parse_available_context_sets_and_index_info(explain_index_json)
    ...
    def test_parse_available_context_set_and_index_info_from_xml_dict_loc(self):
        with open(TestFiles.loc_raw_indexes, "r") as f:
            ...
            sru_dict_parser = SRUExplainDictParser({}, driver=loc_driver)
            available_context_set_and_index_info = sru_dict_parser._parse_available_context_sets_and_index_info(explain_index_json)
    ...
    def test_parse_config_info_alma(self):
        with open(TestFiles.alma_raw_config_info, "r") as f:
            ...
            sru_dict_parser = SRUExplainDictParser({}, driver=alma_driver)
            config_info = sru_dict_parser._parse_config_info(explain_index_json)
    ...
    def test_parse_config_info_loc(self):
        sru_dict_parser = SRUExplainDictParser({}, driver=loc_driver)
        config_info = sru_dict_parser._parse_config_info(None)
    ...
    def test_parse_schema_info_alma(self):
        with open(TestFiles.alma_raw_schemas, "r") as f:
            ...
            parsed_record_schema_info = sru_dict_parser._parse_schema_info(record_schema_info)
    ...
    @patch('src.sru_queryer._base._sru_explain_dict_parser.SRUExplainDictParser._parse_sru_version_from_dict')
    @patch('src.sru_queryer._base._sru_explain_dict_parser.SRUExplainDictParser._parse_raw_index_info_from_dict')
    @patch('src.sru_queryer._base._sru_explain_dict_parser.SRUExplainDictParser._parse_raw_schema_info_from_dict')
    @patch('src.sru_queryer._base._sru_explain_dict_parser.SRUExplainDictParser._parse_raw_config_info_from_dict')
    @patch('src.sru_queryer._base._sru_explain_dict_parser.SRUExplainDictParser._parse_schema_info')
    @patch('src.sru_queryer._base._sru_explain_dict_parser.SRUExplainDictParser._parse_available_context_sets_and_index_info')
    @patch('src.sru_queryer._base._sru_explain_dict_parser.SRUExplainDictParser._parse_config_info')
    ...
    def test_construct_sru_configuration_integration_alma(self):
        with open(TestFiles.explain_response_alma, "rb") as f:
            ...
            sru_dict_parser = SRUExplainDictParser(xml_dict, driver=alma_driver)
            configuration = sru_dict_parser.get_sru_configuration_from_explain_response()
    ...
    def test_construct_sru_configuration_integration_gapines(self):
        with open(TestFiles.explain_response_gapines, "rb") as f:
            with open(TestFiles.gapines_available_context_sets_and_indexes, "r") as f2:
                ...
                xml_dict = xmltodict.parse(f.read())
                sru_dict_parser = SRUExplainDictParser(xml_dict, driver=gapines_driver)
                configuration = sru_dict_parser.get_sru_configuration_from_explain_response()
    ...
    def test_construct_sru_configuration_integration_loc(self):
        with open(TestFiles.explain_response_loc, "rb") as f:
            xml_dict = xmltodict.parse(f.read())
            sru_dict_parser = SRUExplainDictParser(xml_dict, driver=loc_driver)
            configuration = sru_dict_parser.get_sru_configuration_from_explain_response()
